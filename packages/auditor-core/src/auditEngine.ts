...
import { ensureSampleApiRunning, getOrCreateActiveSandbox } from './sandboxManager.js';
...
export async function runAuditOnSampleApi(): Promise<ComplianceReport> {
  ...
  console.log('[Audit] Preparing E2B sandbox...');
  const sandbox = await getOrCreateActiveSandbox();

  // 2. Launch sample API inside sandbox (idempotent)
  console.log('[Audit] Ensuring sample API is running in sandbox...');
  await ensureSampleApiRunning();
