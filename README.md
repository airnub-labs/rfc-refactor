...
3. Wait for the container to build (~2-3 minutes). Docker Compose starts **only** Memgraph + Memgraph MCP (no Next.js container). The workspace container comes from `.devcontainer` using `docker-compose.devcontainer.yml`.
...
# Start graph deps only (no web container in this file)
